 ...
 *
 * Replaces:
 *   - CLI's `uploadBatch()` in apps/agent-dev/src/api.ts (throws on any non-2xx)
 *   - Extension's inline fetch in apps/extension/src/background/ingest-queue.ts
 *
 * Both companions now call `IngestClient.upload(batch)` with identical
 ...
 * companions so "what happens on 401" is answered in exactly one place.
 */
...
/** What a companion provides so the ingest client can mint + refresh
 * bearer tokens. Token storage details stay in the companion
 * (Conf file on Node, IndexedDB on browser). */
export interface AuthProvider {
  getToken(): Promise<string | null>;
  /** Called exactly when the server returns 401/403. The companion
