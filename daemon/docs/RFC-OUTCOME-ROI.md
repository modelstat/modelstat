...
| **Change primitives** | `files_changed`, `lines_added`, `lines_deleted`, `hunks`, and churn split by path class (test / config / doc / generated). | `diff_features` over a bounded `git show --numstat`, `daemon/crates/modelstat-work/src/diff.rs`. Paths are read to classify and then dropped — `DiffFeatures` is deliberately not `Serialize`. | Shipped. |
