...
// prices.json is generated by scripts/build-prices.ts at the repo root;
// if it's missing we keep going but the extension will show $0 until
// someone runs `pnpm extension:build-prices`.
const pricesPath = resolve(extRoot, "public/assets/prices.json");
if (!(await exists(pricesPath))) {
  console.warn(`  warn: ${pricesPath} not found (run \`pnpm extension:build-prices\`)`);
}
