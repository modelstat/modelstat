...
> ⚠️ **This npm MCP server is retired.** Use the hosted server at **https://mcp.modelstat.ai** (OAuth in the browser), or install the daemon — `curl -fsSL https://install.modelstat.ai | sh` — which wires its built-in `modelstat mcp` server into your AI tools automatically. This directory is kept for history only.
