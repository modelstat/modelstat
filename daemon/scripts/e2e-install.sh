...
#   2. idempotent re-run,
#   3. a tampered archive is REFUSED (no partial install).
#
# Fully hermetic — the MODELSTAT_INSTALL_* hooks keep it offline, touch no
# services, and confine every write to a throwaway MODELSTAT_HOME. The live
# curl|sh + real-service path is validated separately on a real machine (M7 AC),
# because service start/stop isn't reliable on CI runners.
set -euo pipefail
...
run_install() { # $1 = throwaway MODELSTAT_HOME
  MODELSTAT_HOME="$1" \
...
H1="$WORK/home1"
run_install "$H1" >/dev/null 2>&1 || fail "install (happy path) exited non-zero"
...
# 2. Idempotent re-run — installing again over the same home still succeeds.
run_install "$H1" >/dev/null 2>&1 || fail "idempotent re-run exited non-zero"
[ -f "$H1/bin/modelstat" ] || fail "collector missing after re-run"
pass "idempotent re-run"

# 3. Tamper — a corrupted archive MUST be refused, leaving nothing staged.
printf 'corrupted' >> "$REL/$ARCHIVE" # sha no longer matches SHA256SUMS
H2="$WORK/home2"
if run_install "$H2" >/dev/null 2>&1; then
